    ...
    if (items.length === 0) {
        return (
            <main className="min-h-screen bg-gray-50 flex justify-center items-center">
                <section className="mx-auto container p-5 flex flex-col items-center">
                    <h1 className="text-center text-xl font-semibold">No wishlisted Items</h1>
                <img src={image} className="object-cover mx-auto mt-10"/>
                <button className="bg-green-400 px-5 py-3 text-xl rounded-lg text-gray-700 font-semibold mt-10 cursor-pointer hover:bg-green-500 active:bg-green-600" onClick={()=> navigate("/")}>Explore Items</button>
                </section>
...
<article className="flex flex-col items-center md:flex-row gap-10 mt-10">
                    {items.map((item) => 
